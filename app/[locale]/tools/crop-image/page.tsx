...
const CropImageTool = dynamic(
  () =>
    import("@/components/tools/CropImageTool").then((module) => ({
      default: module.CropImageTool,
    })),
  {
    ssr: false,
  },
...
export async function generateMetadata({
  ...
  return {
    title:
      params.locale === "zh"
        ? "图片裁剪 - 免费在线图片裁剪工具 - FastTool"
        : "Crop Image | Free Online Image Cropping Tool - FastTool",
    description:
      params.locale === "zh"
...
**3. 图像提取阶段**
- **Canvas 绘制**：使用 Canvas 的 drawImage() 方法从原图中提取指定区域，创建新的 Canvas 元素并绘制裁剪后的图像。
- **像素级操作**：通过精确的坐标计算，确保裁剪区域的准确性。
- **图像导出**：使用 toDataURL() 方法将 Canvas 转换为图片格式，支持多种输出格式和质量设置。
        ...
        useCases: [
          {
            title: "证件照制作",
            description:
              "裁剪照片到标准证件照尺寸（如 1 寸、2 寸），用于简历、申请表、证书等",
          },
          {
            title: "社交媒体头像",
            description:
              "裁剪照片为方形或圆形头像，用于微信、微博、Facebook、LinkedIn 等社交平台",
          },
          {
            title: "产品图优化",
            description:
              "裁剪电商产品图片，突出产品主体，统一店铺图片风格和尺寸",
          },
          {
            title: "照片构图调整",
            description:
              "裁剪照片改善构图，应用三分法则，移除干扰元素，突出主体",
          },
          {
            title: "视频封面制作",
            description:
              "裁剪图片为 16:9 或其他视频封面比例，用于 YouTube、B 站、抖音等平台",
          },
          {
            title: "文档插图处理",
            description:
              "裁剪图片适配 Word、PPT 等文档版面，提升文档专业性和美观度",
          },
        ],
        tips: [
        ...
        faq: [
          {
            question: "裁剪会损失画质吗？",
            answer:
              "裁剪本身不会损失画质，只是提取原图的一部分。但如果裁剪后还要放大图片，可能会导致画质下降。建议裁剪时使用较高的输出质量参数（90% 以上），并在裁剪前确认好尺寸和比例。",
          },
          {
            question: "支持哪些裁剪比例？",
            answer:
              "我们支持自由比例裁剪和多种预设比例：1:1（方形）、4:3（标准）、3:2（经典）、16:9（宽屏）、16:10（黄金）、2:3（人像）、3:4（竖版）、9:16（全屏）等。您也可以自定义任意比例。",
          },
          {
            question: "可以旋转裁剪吗？",
            answer:
              "是的，工具支持旋转裁剪功能。您可以先旋转图片到合适角度，再进行裁剪。这对于校正倾斜的照片特别有用。旋转角度范围为 -180° 到 +180°。",
          },
          {
            question: "裁剪后的图片尺寸是多少？",
            answer:
              "裁剪后的图片尺寸等于您选择的裁剪区域的像素尺寸。如果您在原图上选择了 800x600 像素的区域，裁剪后的图片就是 800x600 像素。您可以在裁剪过程中实时看到裁剪区域的尺寸。",
          },
          {
            question: "可以批量裁剪吗？",
            answer:
              "是的，工具支持批量裁剪功能。您可以一次上传多张图片，对每张图片应用相同或不同的裁剪设置。批量裁剪可以大幅提高工作效率，特别适合处理大量图片。",
          },
          {
            question: "我的图片会被上传吗？",
            answer:
              "绝对不会。所有裁剪操作都在您的浏览器本地完成，图片数据不会离开您的设备。这是我们的核心隐私保护特性，确保您的图片 100% 安全。",
          },
          {
            question: "支持哪些图片格式？",
            answer:
              "支持所有主流图片格式，包括 JPG、JPEG、PNG、WebP、BMP 等。无论您的图片是什么格式，都可以正常裁剪。输出格式可选择 JPG、PNG 或 WebP。",
          },
          {
            question: "裁剪后可以撤销吗？",
            answer:
              "可以。在点击开始裁剪按钮之前，您可以随时撤销或修改裁剪区域。我们建议先预览裁剪效果，确认满意后再执行裁剪操作。裁剪完成后如需重新裁剪，可重新上传图片。",
          },
          {
            question: "如何确保裁剪精确？",
            answer:
              "我们提供网格辅助线（三分法、黄金分割等）帮助您精确构图。您还可以放大图片进行精细调整，查看实时尺寸显示，确保裁剪区域精确到像素级别。",
          },
        ],
      }
    : {
...
- **Canvas Drawing**: Use Canvas's drawImage() method to extract the specified area from the original image, create a new Canvas element, and draw the cropped image.
- **Pixel-level Operations**: Ensure precision through accurate coordinate calculations.
- **Image Export**: Convert Canvas to image format using toDataURL() method, supporting multiple output formats and quality settings.
...
- **Color Fidelity**: Ensure color consistency before and after cropping`,
        ...
        useCases: [
          {
            title: "ID Photo Creation",
            description:
              "Crop photos to standard ID photo sizes (1 inch, 2 inch, etc.) for resumes, applications, certificates",
          },
          {
            title: "Social Media Avatars",
            description:
              "Crop photos to square or circular avatars for WeChat, Weibo, Facebook, LinkedIn, and other social platforms",
          },
          {
            title: "Product Image Optimization",
            description:
              "Crop e-commerce product images to highlight products and unify store image style and dimensions",
          },
          {
            title: "Photo Composition Adjustment",
            description:
              "Crop photos to improve composition, apply rule of thirds, remove distracting elements, highlight subjects",
          },
          {
            title: "Video Thumbnail Creation",
            description:
              "Crop images to 16:9 or other video thumbnail ratios for YouTube, Bilibili, TikTok, and other platforms",
          },
          {
            title: "Document Illustration Processing",
            description:
              "Crop images to fit Word, PowerPoint, and other document layouts for professionalism and aesthetics",
          },
        ],
        tips: [
        ...
        faq: [
          {
            ...
            answer:
              "Cropping itself doesn't affect quality - it just extracts part of the original image. However, if you enlarge the image after cropping, quality may degrade. We recommend using high output quality parameters (90%+) and confirming dimensions and ratio before cropping.",
          },
          {
            question: "Which cropping ratios are supported?",
            answer:
              "We support free ratio cropping and multiple preset ratios: 1:1 (square), 4:3 (standard), 3:2 (classic), 16:9 (widescreen), 16:10 (golden), 2:3 (portrait), 3:4 (vertical), 9:16 (full screen), etc. You can also customize any ratio.",
          },
          {
            question: "Can I rotate while cropping?",
            answer:
              "Yes, the tool supports rotated cropping. You can rotate the image to a suitable angle before cropping. This is particularly useful for correcting tilted photos. Rotation angle ranges from -180° to +180°.",
          },
          {
            question: "What size will the cropped image be?",
            answer:
              "The cropped image size equals the pixel dimensions of your selected crop area. If you select an 800x600 pixel area on your original image, the cropped image will be 800x600 pixels. You can see the crop area dimensions in real-time during cropping.",
          },
          {
            question: "Can I crop images in batch?",
            answer:
              "Yes, the tool supports batch cropping. You can upload multiple images at once and apply same or different crop settings to each image. Batch cropping greatly improves work efficiency, especially for processing large quantities of images.",
          },
          {
            question: "Will my images be uploaded?",
            answer:
              "Absolutely not. All cropping operations happen locally in your browser - image data never leaves your device. This is our core privacy protection feature, ensuring 100% security of your images.",
          },
          {
            question: "Which image formats are supported?",
            answer:
              "All major image formats are supported including JPG, JPEG, PNG, WebP, BMP, etc. Whatever format your image is, it will crop normally. Output format can be chosen from JPG, PNG, or WebP.",
          },
          {
            question: "Can I undo after cropping?",
            answer:
              "Yes. Before clicking the 'Start Cropping' button, you can undo or modify the crop area at any time. We recommend previewing the crop effect first and confirming satisfaction before executing the crop operation. After cropping is complete, if you need to recrop, you can re-upload the original image.",
          },
          {
            question: "How to ensure precise cropping?",
            answer:
              "We provide grid guidelines (rule of thirds, golden ratio, etc.) to help you compose precisely. You can also zoom in on the image for fine adjustments, view real-time dimension displays to ensure crop area is precise to the pixel level.",
          },
        ],
      };
  ...
  return (
      ...
      howToSteps={
        params.locale === "zh"
          ? [
              ...
              "选择裁剪模式：自由裁剪、固定比例或预设尺寸",
              "拖动裁剪框调整裁剪区域，可调整大小、位置和旋转角度",
              "实时预览裁剪效果，可查看裁剪区域的精确尺寸",
              "满意后点击「开始裁剪」按钮，浏览器将在本地执行裁剪",
              "预览裁剪结果，点击下载保存到本地设备",
            ]
          : [
              "Click the upload area or drag and drop images onto the page, supports JPG, PNG, WebP and more",
              "Choose cropping mode: free cropping, fixed ratio, or preset size",
              "Drag the crop box to adjust crop area - can adjust size, position, and rotation angle",
              "Preview crop effect in real-time and view exact dimensions of crop area",
              "Click the 'Start Cropping' button when satisfied - your browser will perform the crop locally",
              "Preview cropped result and click download to save to your device",
            ]
      }
      howToDetail={
        params.locale === "zh"
    ...
    >
      <CropImageTool locale={params.locale} />
    </ToolLayout>
  );
}

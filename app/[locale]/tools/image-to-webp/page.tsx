...
export default function ImageToWebpPage({ params }: { params: { locale: string } }) {
  ...
  return (
    <ToolLayout
      ...
      title={params.locale === "zh" ? "图片转 WEBP" : "Image to WEBP"}
      description={
        params.locale === "zh"
          ? "将各种图片格式快速转换为 WEBP 格式。"
          : "Quickly convert various image formats to WEBP."
      }
      introduction={
        params.locale === "zh"
          ? "该工具使用浏览器端 Canvas API 进行格式转换，在不上传原图的前提下完成转换，适合网站优化和减少图片体积。"
          : "This tool uses browser-side Canvas API to convert images without uploading originals, ideal for web optimization and reducing image size."
      }
      howToSteps={
        params.locale === "zh"
          ? ["上传图片", "点击开始转换", "下载转换后的 WEBP 图片"]
          : ["Upload your image", "Click convert", "Download the converted WEBP image"]
      }
      faq={[
        {

...
export function ImageToIcoTool({ locale }: Props) {
  ...
  const convertToIco = async (file: File): Promise<File> => {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        // ICO 格式通常使用 16x16, 32x32, 64x64 等尺寸
        ...
        canvas.toBlob((blob) => {
          ...
          const convertedFile = new File([blob], `${file.name.split('.')[0]}.ico`, { type: 'image/x-icon' });
    ...
    try {
      ...
      const convertedResults = await Promise.all(
        uploaded.map(async (item) => {
          const converted = await convertToIco(item.file);
              ...
              {results.map((result, index) => (
                <div key={index} className="space-y-2">
                  ...
                  <DownloadButton
                    ...
                    href={result.url}
                    filename={`converted-${result.originalFile.name.split('.')[0]}.ico`}
                    className="w-full"
                  />
                </div>
              ))}
      ...
      <div className="space-y-6">
        <SettingsPanel title={t.tool.settings}>
          <p className="mb-3 block text-sm text-gray-700">
            {locale === "zh" ? "转换为 ICO 格式" : "Convert to ICO Format"}
          </p>
          <p className="mb-4 text-xs text-gray-600">
            {locale === "zh" ? "ICO 格式适用于网站 favicon 和应用图标。" : "ICO format is suitable for website favicons and application icons."}
          </p>
        ...
        {results.length > 0 && (
          <div className="space-y-4">
            ...
            <p className="text-xs text-gray-600">
              {locale === "zh" ? "点击下载每张转换后的图标" : "Click to download each converted icon"}
            </p>
          </div>
        )}
      </div>

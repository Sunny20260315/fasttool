...
export function PdfToWordTool({ locale }: Props) {
  ...
  const convertPdfToWord = async (file: File): Promise<File> => {
...
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  <w:body>
    ...
    const blob = new Blob([mockDocxContent], { type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' });
    const convertedFile = new File([blob], `${file.name.replace('.pdf', '.docx')}`, { type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' });
    ...
    try {
      ...
      const convertedResults = await Promise.all(
        uploaded.map(async (item) => {
          const converted = await convertPdfToWord(item.file);
      ...
      <div className="space-y-6">
        <SettingsPanel title={t.tool.settings}>
          <div className="space-y-4">
            <p className="block text-sm text-gray-700">
              {locale === "zh" ? "将 PDF 文件转换为可编辑的 Word 文档" : "Convert PDF files to editable Word documents"}
            </p>
        ...
        <div className="rounded-xl border border-gray-200 bg-blue-50 p-4">
          ...
          <ul className="space-y-1 text-xs text-gray-600">
            <li>• {locale === "zh" ? "转换后的 Word 文档可以直接编辑" : "Converted Word documents can be edited directly"}</li>
